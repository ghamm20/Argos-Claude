...
10a. **Vault self-heal trust boundary (DECIDED 2026-06-10, owner rider — chunk-level integrity):** The vault manifest self-heals by rebuilding from on-disk chunk files (Phase 8). The trust anchor is the hash-chained audit log: each chunk file's sha256 is recorded in its `vault.ingested` entry at ingest, and VERIFIED at heal. A chunk that is tampered (hash mismatch) or has no `vault.ingested` provenance (planted) is QUARANTINED to `vault/index/quarantine/` and audited (`vault.chunk_quarantined`) — never silently indexed. Grace: a provenance-less vault (copied without `state/`) falls back to operator-local-disk trust and audits `vault.heal_unverified` (no wholesale quarantine of a legitimate copy); legacy chunks ingested before the hash existed are trusted by provenance-existence. So a planted chunk only becomes canon if it ALSO has a forged audit-chain entry — which the chain's tamper-evidence resists.
